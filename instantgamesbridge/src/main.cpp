#define EXTENSION_NAME instantgamesbridge
#define LIB_NAME "instantgamesbridge"
...
extern "C" void InstantGamesBridge_RegisterCallback(ObjectMessage cb_string);
extern "C" void InstantGamesBridge_RemoveCallback();
extern "C" void InstantGamesBridge_Init(const char* parameters, const int callback_id);
extern "C" char* InstantGamesBridge_CallApi(const char* method, const char* parameters, const int callback_id, const bool native_api);
...
static void InstantGamesBridge_SendStringMessage(const int callback_id, const char* message)
{
...
static int Init(lua_State* L)
{
    ...
    InstantGamesBridge_Init(luaL_checkstring(L, 1), callback_id);
    return 0;
}
...
static int CallApi(lua_State* L)
{
    ...
    const char* result = InstantGamesBridge_CallApi(luaL_checkstring(L, 1), lua_isnoneornil(L, 2) ? "" : luaL_checkstring(L, 2),
...
{
    LuaInit(params->m_L);
    InstantGamesBridge_RegisterCallback((ObjectMessage)InstantGamesBridge_SendStringMessage);
    return dmExtension::RESULT_OK;
}

static dmExtension::Result FinalizeExtension(dmExtension::Params* params)
{
    InstantGamesBridge_RemoveCallback();
    return dmExtension::RESULT_OK;
}
